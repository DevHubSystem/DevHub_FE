...
const navItem = 'flex h-[38px] w-full items-center gap-3 rounded-lg px-[11px] text-sm font-medium whitespace-nowrap transition-colors'
...
const subItem = 'ml-3.5 flex h-[34px] w-full items-center gap-2.5 rounded-[7px] border-l border-[#e3e7ee] pl-3.5 pr-[11px] text-[13.5px] whitespace-nowrap transition-colors dark:border-white/[0.07]'
 ...
 *   activeSpace: string|null, setActiveSpace: (id: string) => void }} props
 */
const Sidebar = ({ collapsed, view, setView, spacesOpen, setSpacesOpen, activeSpace, setActiveSpace }) => {
  return (
    <aside className={cn('flex flex-col overflow-hidden border-r border-[#e3e7ee] bg-[#eef1f5] transition-[width] duration-200 dark:border-white/[0.07] dark:bg-[#0c0f13]', collapsed ? 'w-16' : 'w-[248px]')}>
      <div className={cn('flex-1 overflow-y-auto overflow-x-hidden pb-5 pt-2.5', collapsed ? 'px-0' : 'px-3')}>
        ...
        {collapsed ? (
          ...
          <Collapsible open={spacesOpen} onOpenChange={setSpacesOpen}>
            ...
            <CollapsibleContent>
              <div className="py-1.5 pl-[26px] text-[11px] font-bold uppercase tracking-[0.07em] text-[#8b95a4] dark:text-[#6b7684]">
                Recent
              </div>
              {SPACES.map((s) => (
                <button
